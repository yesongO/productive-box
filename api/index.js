...
;// CONCATENATED MODULE: ./dist/generateBarChart.js
/**
...
;// CONCATENATED MODULE: ./dist/githubQuery.js
/* harmony default export */ async function githubQuery(query) {
...
;// CONCATENATED MODULE: ./dist/queries.js
const userInfoQuery = `
  query {
    viewer {
      login
...
(async () => {
     ...
     * First, get user id
     */
    const userResponse = await githubQuery(userInfoQuery).catch((error) => console.error(`Unable to get username and id\n${error}`));
    ...
    const repoResponse = await githubQuery(contributedRepoQuery).catch((error) => console.error(`Unable to get the contributed repo\n${error}`));
    /**
     ...
     */
    const committedTimeResponseMap = await Promise.all(repos.map(({ name, owner }) => githubQuery(createCommittedDateQuery(id, name, owner)))).catch((error) => console.error(`Unable to get the commit info\n${error}`));
     ...
     */
    const octokit = new dist_node.Octokit({ auth: `token ${process.env.GH_TOKEN}` });
